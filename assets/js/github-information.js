function fetchGitHubInformation(event) {
    ...
    $when(
        $.getJSON(`https://api.gihub.com/user/${username}`),
        $.getJSON(`https://api.gihub.com/user/${username}/repos`)
    )
    .then(
        (firstResponse, secondResponse) => {
            ...
            if (errorResponse.status === 404) {
                ...
                $("gh-user-data").html(`<h4>Too many requests, please wait until ${resetTime.toLocaleTimeString} and try again.</h4>`);
            } else {
